...
const findAllReportAttendance = async (req, res) => {
  const { error, value } = (0, validateRequest_1.validateRequest)(
    attendanceSchema_1.findAllAttendanceSchema,
    req.query
  )
  if (error != null) {
  ...
  try {
    const page = new pagination_1.Pagination(
      parseInt(queryPage) ?? 0,
      parseInt(querySize) ?? 10
    )
      ...
      where: {
        ...
        attendanceTime: {
          ...
          [sequelize_1.Op.between]: [
            `${startQueryDate} 00:00:00`,
            `${endQueryDate} 23:59:59`
          ]
        }
      },
    ...
    schedules.forEach((schedule) => {
      ...
      const scheduleDate = (0, moment_1.default)(schedule.scheduleStartDate).format(
        'YYYY-MM-DD'
      )
    ...
    attendances.forEach((attendance) => {
      ...
      const attendanceDate = (0, moment_1.default)(attendance.attendanceTime).format(
        'YYYY-MM-DD'
      )
